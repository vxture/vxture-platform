...
echo ""
echo "==> [3/4] 拉取镜像（docker compose pull）"
cd "$COMPOSE_DIR"
VX_IMAGE_REGISTRY="$IMAGE_REGISTRY" VX_IMAGE_NAMESPACE="$IMAGE_NAMESPACE" VX_IMAGE_TAG="$IMAGE_TAG" docker compose -f compose.platform.yml pull

# ── 启动 ─────────────────────────────────────────────────────────────────────

echo ""
echo "==> [4/4] 启动平台栈（docker compose up -d --remove-orphans）"
VX_IMAGE_REGISTRY="$IMAGE_REGISTRY" VX_IMAGE_NAMESPACE="$IMAGE_NAMESPACE" VX_IMAGE_TAG="$IMAGE_TAG" docker compose -f compose.platform.yml up -d --remove-orphans
