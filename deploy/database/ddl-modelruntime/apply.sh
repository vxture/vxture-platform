...
# 按文件名序 apply *.sql：00_schemas → 10_key/20_reqlog/30_routing/40_model → 90_partitions → 95_triggers。
...
if [[ "${1:-}" == "--reset" ]]; then
  echo "⚠ --reset: dropping key/reqlog/routing/model (CASCADE). Data loss."
  [[ "${CONFIRM_RESET:-}" == "yes" ]] || { echo "refusing: set CONFIRM_RESET=yes"; exit 1; }
  "${PSQL[@]}" -c "DROP SCHEMA IF EXISTS key,reqlog,routing,model CASCADE;"
fi
