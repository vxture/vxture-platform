 ...
 * stack is never logged, hiding the real fault (e.g. a first-login throw on the
 * RP callback or /api/me). This filter logs the method + URL + full stack for
 * every 5xx / unexpected throw, and returns a clean JSON error (no internals
 * leaked). HttpExceptions keep their original status + body.
 */
import {
  ArgumentsHost,
  Catch,
