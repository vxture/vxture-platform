...
function classify(files, extraArgs = []) {
  const out = execFileSync(process.execPath, [SCRIPT, '--files', files.join(','), ...extraArgs], {
    encoding: 'utf8',
  });
  const result = {};
...
function builtImages(files) {
  return ALL_IMAGES.filter((image) => classify(files, ['--image', image]).image_build === 'true');
}
...
test('image set: portals/admin → 仅 admin', () => {
  assert.deepEqual(builtImages(['portals/admin/src/x.tsx']), ['admin']);
});
...
test('image set: bff/auth-bff → 仅 bff-auth', () => {
  assert.deepEqual(builtImages(['bff/auth-bff/src/x.ts']), ['bff-auth']);
});
...
test('global rule: docker-build.yml → 全部镜像', () => {
  assert.deepEqual(builtImages(['.github/workflows/docker-build.yml']), ALL_IMAGES);
});
...
test('matrix: portals/admin → 仅 admin，且携带完整构建配置', () => {
  ...
  assert.equal(include[0].image, 'ghcr.io/vxture/admin');
...
test('matrix include 镜像名与 ALL_IMAGES 一致（单一数据源对齐）', () => {
  const include = JSON.parse(classify(['pnpm-lock.yaml'], ['--matrix']).matrix).include;
  assert.deepEqual(
    include.map((entry) => entry.name),
    ALL_IMAGES
  );
