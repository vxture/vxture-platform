...
function loadWorkspacePackages() {
  ...
  const roots = ['services', 'bff', 'packages', 'portals', 'agent-server', 'agent-studio'];
  const walk = (dir) => {
    let entries;
    try {
...
function buildImageRules() {
  ...
  for (const img of IMAGES) {
    ...
    if (img.name === 'bff-gateway') prefixes.add('bff/gateway-bff/');
    rules.set(img.name, [
      {
...
function listChangedFiles(baseSha, headSha) {
  ...
  if (isUsableSha(baseSha)) {
    try {
      return splitLines(runGit(['diff', '--name-only', baseSha, normalizedHead]));
    } catch {
  ...
  try {
    return splitLines(runGit(['diff', '--name-only', `${normalizedHead}^`, normalizedHead]));
  } catch {
...
function matchesRule(filePath, rule) {
  ...
  return exact.includes(filePath) || prefixes.some((prefix) => filePath.startsWith(prefix));
}
...
const outputFile = options.get('github-output') ?? process.env.GITHUB_OUTPUT ?? '';
...
if (aggregate) {
  ...
  const deployChanged = changedFiles.some(
    (filePath) => filePath.startsWith('deploy/') && !filePath.startsWith('deploy/docker/')
  );
