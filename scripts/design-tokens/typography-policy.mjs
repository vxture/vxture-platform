 ...
 *   字距  **一律取字号档自带的字距**。字距是字号的函数不是角色的函数——曲线与
 *         依据见 foundation-policy。唯一的角色级例外是全大写，那与尺寸无关。
 *
 ...
 * 指南里最普适的一条，故只能留在角色层。`overline` 按定义是标题上方的全大写微型
 * 标签，**使用它的组件需自行加 `uppercase`**；T2 的 `--text-*` 机制落不了
 * text-transform。
 */
 ...
 *
 * 两项修正：
 * - 行高 +0.15。汉字是全高方块，没有西文的升降部留白，按拉丁行高排会挤成一片。
 *   中文正文的通行区间是 1.7–1.8，拉丁是 1.5。
 * - 字距 +0.02em。汉字字面本就填满字身，字距为负会直接相撞；轻微放开还能改善
 *   长段中文的辨识。
 *
 * ⚠ 写成**加法**而不是另一套取值，是为了与字号三档正交：修正量叠在档位曲线上，
 *   三个模式各自的字号档不必再复制一遍。副作用正好是想要的——大字号的拉丁收紧
 *   （−0.025em）被加回到接近零，而那正是汉字需要的。
 *
 * 由 `:lang(zh)` 自动生效：文种是内容属性，不该要求业务显式声明。
 */
export const CJK_SELECTOR = ":lang(zh)";
export const CJK_LEADING_ADD = 0.15;
export const CJK_TRACKING_ADD = "0.02em";
