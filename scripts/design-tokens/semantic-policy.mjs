...
export const Z_LADDER = [
  ...
  [
    "dropdown",
    650,
    "高于 popover——ActionMenu 这类 portal 化下拉菜单常挂在表格行操作上，而表格常年出现在 Drawer/Dialog 内部（如详情抽屉的行操作），下拉必须能压过触发它的任意容器；同时仍压过粘性表头。2026-08-12 由 200 上调：原值低于 drawer(400)，Drawer 内的 ActionMenu portal 后被抽屉自身盖住，点了没反应（products 页 OIDC 客户端抽屉行操作实测复现）",
  ],
...
export const PANEL_WIDTHS = [
  ...
  // owner 批准新增（2026-08-18，admin 取值层拆桥批七）：admin 的 xl 对话框
  // 实值 58rem，与 lg 之间差 16rem——这不是吸附误差是缺挡。±16rem 一档的
  // 步长与 lg−md=10rem 同量级递增，件为超宽表单/双栏预览。
  ["xl", "58rem", "超宽对话框：双栏表单、并排预览"],
];
