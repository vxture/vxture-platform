...
| 件    | 位置                                                    | 说明                                                                                                                                |
| ----- | ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| 前端  | `portals/capconsole`(`vx-platform-capconsole`:3050)     | Next.js 薄壳:workforce 会话头 + 侧栏导航 + 总览页;复用 shell-template CSS;**零本地 CSS、零 NEXT_PUBLIC 域名**(BFF 一律同源相对路径) |
| BFF   | `bff/capconsole-bff`(`vx-platform-capconsole-bff`:3051) | workforce realm OIDC RP(client_id=`capconsole`)+ nginx `auth_request` 门 + operator-OBO 换票(M-1);RP 会话 Redis,默认 TTL 12h;不连库 |
| vhost | `deploy/nginx/templates/capconsole.vhost.template`      | 20-sync 脚本从 `.env.capconsole-bff` 的 `CAPCONSOLE_BASE_URL` 渲染真实 `server_name`;env 缺失即跳过                                 |
...
5. **接入步骤**(平台侧,每模块一次):
   - `bff/capconsole-bff/src/routers/oidc-auth.router.ts` 的 `MODULE_AUD_BY_PREFIX` 加一行;
   - vhost 模板取消对应 `location /{code}/` 注释,填模块的 tailnet 地址(worker-02 等);
   - `portals/capconsole/src/config/navigation.ts` 对应项 `pending` 置 false。

## 3. 部署序(批C 上产时,owner-gated)

1. `27-provision-client-secrets.sh`(capconsole 已入 CLIENTS_ALL)→ `.env.capconsole-bff` 得 `OIDC_CLIENT_SECRET`,`.env.auth-bff` 得 hash;
2. 主机 `.env.capconsole-bff` 填真实 `CAPCONSOLE_BASE_URL`(参照 `deploy/.env.capconsole-bff.example`);
3. reseed(23/29,`appoidc.oidc_clients` 落 capconsole redirect;两脚本已白名单该 env);
4. `20-sync-nginx-config.sh`(渲染 vhost + default-server 兜底)+ compose up capconsole/capconsole-bff;
5. 收尾:`deploy/guardrails/39-audit-env.mjs` 把 capconsole-bff 条目 `requiredActual` 翻成 `STRICT_RUNTIME`、`OIDC_CLIENT_SECRET_HASH_CAPCONSOLE` 移入 required 集(首部署后 fail-closed)。
