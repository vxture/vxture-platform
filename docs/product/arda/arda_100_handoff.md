...
> 性质：**汇编与索引**——语义以权威文档为准。接口契约本体 = [`arda_200_interface.md`](arda_200_interface.md)；最终对接要求与决策留痕 = [`arda_300_integration-final.md`](arda_300_integration-final.md)；产品定义 = [`arda_000_definition.md`](arda_000_definition.md)。
...
| 文件                                                                                          | 作用                                                                                                             |
| --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| [`arda_000_definition.md`](arda_000_definition.md)                                            | Arda 产品定义——v1 功能切片依据（目录四元组、连接器类型；不含 P 级供给/跨 WS 共享）                               |
| [`arda_200_interface.md`](arda_200_interface.md)                                              | **本产品对接接口契约**：C1/C2/C3/webhook 端点、请求响应形状、值域、鉴权——散落各权威的接口点收敛成一份可实现契约  |
| [`arda_300_integration-final.md`](arda_300_integration-final.md)                              | **最终对接要求 + 决策留痕**：三通道定型后的产品侧义务清单 + 关键架构决策（含被否决的旧方案，防回退）             |
| [`product_200_integration.md`](../../design/product_200_integration.md)                       | 三通道对接契约权威；§7 = 新产品接入 checklist（验收清单）                                                        |
| [`product_220_catalog-resource-model.md`](../../design/product_220_catalog-resource-model.md) | 目录·权益与资源模型权威：C2 信封形状、L0 共享资源目录、消费方义务                                                |
| [`identity-platform-rp-integration.md`](../../design/identity-platform-rp-integration.md)     | C1 RP 接入通则（五端点、PKCE、RS256 验签、服务端会话、back-channel logout）；§5 = provisioning webhook wire 契约 |
| [`data_platform_100_architecture.md`](../../design/data_platform_100_architecture.md) §2.3    | 业务面模板硬约束：库名/四 schema/`workspace_id` 隔离键/不读平台库                                                |
| [`product_210_tool-protocol.md`](../../design/product_210_tool-protocol.md)                   | L0 工具协议（"agent 调 Arda"目标态，v1 不要求实现，仅预留见 §4）                                                 |
...
| 鉴权    | 请求头 `x-vxture-internal-auth`（= 平台 `AUTH_INTERNAL_TOKEN`，arda 侧键名 `PLATFORM_INTERNAL_AUTH_TOKEN`，owner 手动转运）——过渡 S2S 凭证，[`product_210`](../../design/product_210_tool-protocol.md) token exchange 落地后迁移，端点契约不变 |

信封形状、值域、门控公式见 [`arda_200_interface.md`](arda_200_interface.md) §2/§5。
