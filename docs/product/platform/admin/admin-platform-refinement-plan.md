...
- **文档陈旧误导级**：docs/packages/bff/admin.md 记载 4 个已不存在端点、缺 15+ 现存 router，登录链路描述整体过期（Batch 8 已 OIDC RP 化，Turnstile 归 auth-bff/accounts 侧，照文档配置会被 39-audit-env 拦部署）；docs/design/auth.md §12、docs/ai/05、docs/packages/index.md 同口径残留；admin/index.md（05-11）与 docs/status.md（06-10）状态表滞后；workplans/2026-06-02-admin-turnstile-independent-surface.md 剩余项落点已失效应关闭。
...
- docs/packages/bff/admin.md 按现行 30 router 整页重写（OIDC RP 化、\_\_Host-vx_rp_session）；
- docs/design/auth.md §12、docs/ai/05-bff-data-access-guide.md、docs/packages/index.md 的 Turnstile/签发旧口径修正；
- docs/product/platform/admin/index.md 状态表回填（B1–B18/TD-021 后实况）；docs/status.md T12/T15 销号；
...
- tech-debt 登记 [TD-026](../../../tech-debt.md#td-026) 并同步销号。
