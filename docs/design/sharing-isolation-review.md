...
现有体系自上而下是：`control-plane.md`（双平面技术架构）→ `data_platform_100`（数据架构权威）→ `data_*_200`（字段级）。**缺一层"平台是什么产品组合、产品之间什么关系"的产品架构层**——v0.2 正好补这一层，位于 control-plane 之上、作为各产品定义（`docs/product/`）与各域数据设计的共同上游。
...
1. **请 owner 提供两份上游文档**，与 v0.2 原稿一并 commit 进 `docs/design/inputs/`；
...
1. 上游 `product-matrix.md` + `ADR-entitlement-and-workspace.md` 由 owner 提供，与 v0.2 原稿一并 commit 进 `docs/design/inputs/`（吸纳硬规，阻塞项）；
2. §8 拍板批处理 → owner 出 v0.3（含 §6 修订）→ 转正为 `docs/design/platform-sharing-isolation.md`（产品架构层权威，与 control-plane.md 同级互引）。
