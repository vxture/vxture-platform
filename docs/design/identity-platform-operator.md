...
> - `docs/design/identity-operator-access-security.md`（v1.0，WebAuthn-first + mTLS 路线，已被 V2 决策推翻）
> - `docs/design/identity-sso-p2-admin.md`（operator realm RP 详细设计 → 仍有效部分并入本文 §3/§4/§5/§9）
...
- **已删除**（吸纳/取舍完成）：`Operator-Identity-Security-Design-V2.md`、`admin-access-security-design.md`、`docs/design/identity-operator-access-security.md`、`docs/design/identity-sso-p2-admin.md`、`docs/deployment/operator-access-cloudflare.md`（mTLS 运维手册，随 Cf-Access 彻底移除）。
- **已更新交叉引用**：`docs/design/data_platform_200_schema.md` §14（ops 表清单→operator\_\*）；本文 §6（operator MFA/账号"延后/复用 ops.admin"→"见本文 §6"）。
