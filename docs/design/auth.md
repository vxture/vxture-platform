...
- env 文件所有权：tenant Turnstile secret 只放 `/srv/vxture/runtime/.env.auth-bff`；admin Turnstile secret 只放 `/srv/vxture/runtime/.env.admin-bff`；本地真实值对应放在 `runtime/VXTURE_DEPLOY_HOST`；前端 site key 只通过 CI 构建变量注入。详见 [`docs/deployment/01-environments.md`](../deployment/01-environments.md)。
...
完整变量清单以 [`docs/deployment/01-environments.md`](../deployment/01-environments.md) 为准。本设计文档只保留认证域相关归属规则：
