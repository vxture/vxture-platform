...
> 版本：v1.0（2026-06-15）
> 状态：设计（待实施）。从属于权威设计 `docs/design/identity-platform-architecture.md`（§3 会话拓扑、§4 认证、§5 iam）；与之冲突以权威设计为准。
...
4. **对外契约**：按本文规则重写 `docs/design/identity-platform-ruyin-contract.md`（ruyin）/ `-p4-app-integration-contract.md`（通用）。
5. IdP 侧：**零改动**。
