...
| 文档                                                            | 关系                                                                                                 | 处理                                            |
| --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ----------------------------------------------- |
| `docs/design/auth.md`                                           | **被本文取代其机制部分**（自定义 JWT、HS256、跨域桥）；账号/PLG/双账号体系等不变                     | 实施期逐节迁移引用，最终标注 superseded-by 本文 |
| `docs/design/session.md`                                        | **被取代**（cookie 桥 → OIDC 会话与 back-channel logout）                                            | 同上                                            |
| `docs/design/tenant.md`                                         | **对齐**（active_tenant 由全局改为按应用，见 §7）                                                    | 增补 per-app 语义                               |
| `docs/design/identity-platform-authorization.md`                | **对齐**（token 内只放治理角色，细粒度回查不变，见 §8）                                              | 增补 token/回查边界                             |
| `docs/design/data_platform_100_architecture.md` / `commerce.md` | **对齐 + 增量**（新增 oidc_client / auth_session / provisioning + commerce 改 per-app 订阅，见 §13） | 按 §13 落详细设计                               |
| `docs/decisions/001-auth-bff-sole-jwt-issuer.md`                | **延续**（auth-bff 仍是唯一签发者，只是改为 OIDC IdP + 非对称签名）                                  | 新增 ADR 记录算法迁移                           |
| `docs/db/schemas/*.sql`                                         | ⚠️ **过期**（仍是迁移前 `account.*`/`tenancy.*` DDL，而生产已是 `identity.*`/`tenant.*`/`iam.*`）    | 文档卫生：本轮一并校正或标注废弃                |
...
| **P5 收尾**                | 退役 HS256 共享密钥路径；校正 `docs/db/schemas/*.sql`；auth.md/session.md 标 superseded                                                                                           | 全平台仅 OIDC；旧密钥下线                                    |
