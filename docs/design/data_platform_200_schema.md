...
> 权威专项：运营身份安全的字段级权威设计在 `docs/design/identity-platform-operator.md`（下称"专项 §6"）。**本章不复制专项**，只做三件事：(a) 把运营身份域在 v2 总纲里的落库归口讲清（schema=`admin`、与客户 realm 五维硬隔离）；(b) 补齐专项不覆盖的**平台治理表**字段级规格；(c) 收口 schema 命名、表计数、治理表命名等二次分析遗留问题。
