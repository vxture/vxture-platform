...
> 定位:平台传输面(S2S 内网 tailnet 寻址、边缘/内网双面边界、控制·数据面分离)的**架构定稿**。产品侧决策留痕见 [`arda_300`](../product/arda/arda_300_integration-final.md) §2。
...
**先例佐证**:平台 LLM 网关(`100.100.197.42:3100`)已由 worker-02 varda-server 经同一 tailscale 接口生产消费(`deploy/compose.platform.yml`、`docs/deployment/varda-worker-02-runbook.md` §3)——worker-01↔worker-02 内网 S2S 路径**已在生产验证**,arda C2/C3 复用同路径,**零新基建**。
