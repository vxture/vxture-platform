...
| accounts(IdP) | accounts.vxture.com(`/oidc/*`→auth-bff)       | 3060-3079 | vx-platform-accounts:3060 / auth-bff:3061   | auth-bff `<worker-01-tailnet-ip>:3061`(tailnet-only,原 3090)  |
| varda         | 无域名(L0 内嵌,console/admin `/varda/*` 反代) | 3080-3099 | varda-bff:3080 / varda-agent:3081           | `<worker-01-tailnet-ip>:3080/3081`(tailnet-only,原 3121/3122) |
...
| **S2S 内网别名**                             | 内口 `vx-platform-api:3041`,tailnet 绑定 `<worker-01-tailnet-ip>:8080` | **已是外部产品仓的既定契约值**(karda/arda 等 `.env.example` 的 `PLATFORM_API_URL` 直接写死这个 tailnet 地址)——重排会破坏已建立的跨仓契约,和"自己家内部随便改"不是一回事;admin 子块特意让开 3041/3042 两位,避免与它在文档里显得"撞号"(运行时其实互相隔离,不会真冲突,纯粹是登记表可读性考虑) |
| model-platform(旧 `services/model/platform`) | 已退役                                                                 | 2026-07-28 本仓服务删除,cutover 完成;新家(独立仓 `vxture-atlas`)沿用 3100,已在 §3 产品表登记为 L1 值                                                                                                                                                                                       |
