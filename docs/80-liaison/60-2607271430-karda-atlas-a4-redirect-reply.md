...
本函 §3 登记的两条「平台线跟进」**均已完成，但完成后未回填、也未告知 karda**——本函当时的两个前提现已全部不成立：

| 跟进项                                       | 当时状态（2026-07-27）                                                                           | 现状                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| token-exchange 签发端点排期                  | 函中转述 Atlas 原话「platform 侧 token-exchange 端点还没实现」，A4/embed/rerank/parse 验签必失败 | ✅ **已实现并在产**。`POST /oidc/token` 支持 `grant_type=token-exchange`（RFC 8693），见 `bff/auth-bff/src/routers/oidc.router.ts` 的 token endpoint 与 `bff/auth-bff/src/oidc/token-exchange.service.ts`（`TOKEN_EXCHANGE_GRANT_TYPE` / `exchange()`）。四种模式全部在用：T1 OBO、operator-OBO、platform-caller、service。收敛路线见 `product_230` §5（D3） |
| `infra-allocation-registry` Atlas 行确认主机 | 「待分配」                                                                                       | ✅ 主机格 = `worker-02`、状态 = **在产**（`13-infra-allocation-registry.md` §3）。⚠️ **同行 `stack_root` 与 beta 域名两格仍是「待分配」**，未随主机一并闭合                                                                                                                                                                                                  |

**因此本函 §0 那句「即便端点地址确认了，你现在也拿不到能通过验签的 token」已经过期**——现在拿得到。karda 若仍按本函结论搁置 A4 接入，是在依据一份已作废的前提。

**遗留（需 owner 决定）**：是否补一封回函告知 karda「token-exchange 已上线」。这是**外发动作**，本次未执行。

**遗留（本仓内部）**：Atlas 行的 `stack_root` / beta 域名两格待 owner 逐格拍板。
