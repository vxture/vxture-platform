...
> Token 策略（有效期 / 存储位置）、Redis Key 规范及 Cookie 命名详见 [`docs/30-design/session.md`](./session.md)。
...
> Cookie Domain / Cookie Key 命名规范详见 [`docs/30-design/session.md`](./session.md) — §Cookie 命名规范。
...
> 登出边界（单域 / 跨域 / logout_all 边界规则）及跨域 SSO 完整流程详见 [`docs/30-design/session.md`](./session.md) — §登出与 Token 吊销 / §跨域 SSO。
