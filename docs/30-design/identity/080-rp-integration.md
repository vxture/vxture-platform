...
| `OIDC_BACKCHANNEL_ISSUER`               | server-to-server 调用（token 换码 + JWKS 拉取）实际打的地址；未设时回退 `OIDC_ISSUER`。app 跑 docker 时**必须**设成宿主机可达地址（dev 常见 `http://host.docker.internal:3081`），否则容器内 `OIDC_ISSUER` 的 `localhost` 段够不到 auth-bff——见 §2.5。**生产部署在 worker-02 的 app（atlas/runos/karda 等 L1/L2）必须设成 `http://<worker-01-tailnet-ip>:8081`**——worker-02 本身无公网（`13-infra-allocation-registry.md` §1），`https://accounts.vxture.com` 从 worker-02 天然不可达，不是"待验证"而是结构性不可达；`<worker-01-tailnet-ip>:8081` 是既有的 auth-bff tailscale 绑定，是 `product_230` §2 定稿的跨仓 S2S 契约值（2026-08-10 由 3090 更正），已有生产先例（worker-02 varda-server 经同一接口消费 `<worker-01-tailnet-ip>:3100`，见 `product_230` §2 "先例佐证"），零新基建 |
