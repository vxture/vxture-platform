...
均 HttpOnly + Secure + SameSite=Lax（Lax 允许 RP→IdP 顶级跳转携带，ruyin 跨站 SSO 因此成立）。admin 侧 `__Host-vx_rp_session` 另在 y.vxture.com host-only，二者正交。（拓扑权威见 [`identity-platform-access-topology.md`](./110-access-topology.md)。）
