...
> 🧭 架构层见 [`identity-platform-architecture.md`](./identity-platform-architecture.md)（板块定位 / 边界 / 拓扑总览 / 双 realm）。本文 = **RP 接入标准 / 集成模板 / business-app 契约 / provisioning 的详细层 reference**。
...
- IdP 端机制（`/oidc/authorize` 决策、`/oidc/token` 签发、realm/`sub`/会话 cookie、密钥轮换、back-channel 投递）见对侧 [`identity-platform-idp.md`](./identity-platform-idp.md)；本文只述 **RP 侧**运行时装配与契约。
...
读 __Host-vx_rp_session → rpsid → 载 vx:rp:{client_id}:sess:{rpsid}
  ...
  组装 AuthUser{ sub, userType, active_tenant, active_tenant_role/status/env, entitlement, tenants[] }
  跑门控链：account_status==active、active_tenant_status（frozen→只读）、entitlement.status
注入 request 上下文（复用/扩展 @vxture/core-auth 的 AuthUser；TenantContext 由 active_tenant 派生）
```

> **AuthUser claims 字段权威 = b（列/类型），语义与门控链 = a §8。** 本文只描述 RP 侧运行时装配，claim 承载的 `active_tenant` / role / status / env / entitlement / `tenants[]` 定义不在此重述。细粒度 console 权限仍回查 `iam.*`（不进 token）——`iam.*` 表结构字段权威 = b（iam schema），语义 = a §8；业务角色回查应用库。
...
- **access_token**（tenant realm，exp = client TTL）：携带 `iss` / `aud` / `sub:"usr_<uuid>"` / `iat` / `exp` / `jti` / `scope` / `sid` / `userType` + 账号态（`account_status` / `phone` / `phone_verified` / `email` / `email_verified`）+ 租户上下文（`active_tenant` / `active_tenant_type` / `active_tenant_role` / `active_tenant_status` / `active_tenant_env` / `tenants[]`）+ 可选 `entitlement`。

> ⚠️ 命名现状：租户上下文 claim 目前是 `active_tenant*` 系列（数据层已迁 org/workspace，claim 名暂沿用 tenant 口径，`company`→`organization` 映射）。app 按**本文档名**编码，平台若改名将升版本 + 重叠期通知。`sub` 恒 `usr_` 前缀，业务库引用建议存完整 `sub`。`entitlement` **仅当该 client 配置了 `product_ref` 且存在订阅时出现**〔邻域 commerce，§5〕——起步期建议 app 不依赖，业务授权由 app 自有库按 `(active_tenant, sub)` 解析。
...
前端 ─POST /auth/switch-tenant {tenantId}──▶ app-bff
  预检：tenantId ∈ 当前 AuthUser.tenants[]（否则 403）
  302 IdP /oidc/authorize?prompt=none&tenant_hint={tenantId}
  IdP 更新 (sid, client_id)→tenantId → 静默发码 → /auth/callback → 新会话(新 active_tenant)
  前端 reload
```

- **作用域按应用**：IdP 只改 `(sid, client_id)` 的 active_tenant，不波及其它 app（如 console 切租户不影响 ruyin / website）。
...
> 登记方式：平台在 seed catalog 的 oidc_client 列表加该 client（现有 website / console / admin），secret hash 经部署期 `27-provision-client-secrets` 注入；生产 `redirect_uris` 等由对应 `*_BASE_URL` env 派生，须与登记值一致。字段级 DDL / 列 / 索引 / `@@map` / `@@schema` / 初始 seed → **b §5.3** / 落地增量迁移见 [`identity-platform-implementation.md`](./identity-platform-implementation.md)。
...
> ⚠️ **ruyin 契约保持独立、不并入本文**（外部依赖，`ruyin.ai` 在 umbra / worker-04，跨仓）——详见独立契约 [`identity-platform-ruyin-contract.md`](./identity-platform-ruyin-contract.md)。跨域接入拓扑（SSO / SLO / 跨域）另见 [`identity-platform-access-topology.md`](./identity-platform-access-topology.md)。本文仅作指针。
...
- [ ] 切租户（console/xuanzhen）：`prompt=none` + `tenant_hint` 静默换发新 `active_tenant`，**不影响**其它 app。
...
- **Provisioning / entitlement / 开通编排 = 邻域 commerce，起步期 OUT**（§5）：commerce 未上线，建议 app 不置 `product_ref`、业务授权走自有库（按 `(active_tenant, sub)`）；商业化后再启用 token `entitlement` 硬门控 + provisioning 投递闭环。
- **ruyin 契约独立**（§6，外部依赖，不并入）。
- **跨域 SLO 可配置**：是否参与全域 back-channel logout 可逐 app 配置；ruyin 默认参与。
- 不在本文内：operator 接入（运营后台 admin，`workforce` realm，另案）、社交联邦（IdP 内部，见 [`identity-platform-idp.md`](./identity-platform-idp.md) §7 + [`identity-platform-account.md`](./identity-platform-account.md)）、MFA（高权限入口，另案）。
