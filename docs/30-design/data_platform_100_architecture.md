...
> 上级：[`docs/30-design/control-plane.md`](./control-plane.md)（双平面**概念概览**）。**本文 = 平台数据架构唯一顶层权威**——原 `database.md` 顶层内容已并入 §2（拓扑/数据流/铁律/治理）+ §3.2（命名规范），database.md 退役（superseded，见 §0.2）。
...
> **[`docs/30-design/data-access.md`](./data-access.md)**（执行级别：强制）。本表仅列顶层边界。
