...
- **锚点列不可变**（`id` / `*_no` / `created_at` / 安全语义列如 `admin.operator_role.rank`）：
  ...
  - **DB 物理锁（target-state，待前置）**：目标是**列级 `REVOKE UPDATE … GRANT UPDATE(可写列白名单)`** 物理锁死，使服务账号无法 UPDATE 锚点列。**前置未满足**——PostgreSQL 列级权限**对 owner/superuser 无效**，而当前全部服务以 owner `vxture` 连库；须先引入**非-owner 分域服务角色**（`*_svc`）并切连库串，方能生效。见 [`tech-debt.md`](../60-operations/tech-debt.md) **TD-018**。
