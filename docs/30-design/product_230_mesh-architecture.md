...
- **判类精化(平台裁定,较回函 04 收紧)**:两轴各管各的——**S2S 传输走哪张网,判据 = 是否在平台 tailnet**(部署事实);**登录态/cookie 互验可不可行,判据 = eTLD+1**(域名事实)。二者通常一致,但 `anlan.ai`/`xuanzhen.ai` 等异 apex 域名产品若部署在平台 tailnet 内,**S2S 照走类 2 内网**,仅会话互验面按跨域处理(runos 曾是此例,2026-08-09 已迁回 `runos.vxture.com`,ADR-004;`runos.ai` 从不存在)。"类 1/类 2"标签按 tailnet 归属取值,域名仅是缺省信号;
...
| 平台 S2S base | **worker-01 tailnet 的 S2S 换票口**(tailscale 接口 → auth-bff 容器内口；端口见[端口登记表](https://claude.ai/code/artifact/0f44735a-c6bc-4881-a440-3446a2411a5f))——**2026-08-10 更正**:此格原先写的是 auth-bff 的容器内口,而 L0 重排挪过它,契约文档与运行态对不上两周。现改为**契约值只认边缘带**,容器内口怎么重排都不再波及本格,产品仓无须再跟。具体端口见[端口登记表](https://claude.ai/code/artifact/0f44735a-c6bc-4881-a440-3446a2411a5f) |     |
| 寻址形态      | **raw tailscale IP:port = 现行仓内约定**(先例 `MODEL_PLATFORM_URL=http://<worker-01-tailnet-ip>:3100`,`deploy/worker-02/.env.varda-server.example`;tailnet 私网地址无 tailnet 成员身份+凭证不可达,非 secret,仓内公开登记);MagicDNS 未启用,未来启用可平替(改名不改约定)                                                                                                                                                                        |
...
**先例佐证**:平台 LLM 网关(`<worker-01-tailnet-ip>:3100`)已由 worker-02 varda-server 经同一 tailscale 接口生产消费(`deploy/compose.platform.yml`、`docs/50-deployment/varda-worker-02-runbook.md` §3)——worker-01↔worker-02 内网 S2S 路径**已在生产验证**,arda C2/C3 复用同路径,**零新基建**。
...
| 面                           | 主体                     | 路径                                                                                                                          | 边界控制                                 |
| ---------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| **边缘面**(公网 TLS)         | 用户浏览器               | `/auth/*`、页面 `/`、浏览器 `/api/*`、**仅浏览器 well-known** `/.well-known/openid-configuration`                             | OIDC / session cookie;公网可达是**必须** |
| **tailnet 面**(S2S,绝不公网) | 产品 ↔ 平台、产品 ↔ 产品 | C2/C3/gauge/可见集出站、provisioning webhook(§3.1)、产品↔产品取数、产品内部触发端点、**工具发现 `/.well-known/vxture-tools`** | tailnet + S2S 鉴权;**任一字节不出内网**  |

> **`/.well-known/vxture-tools` 归 tailnet 面(owner 2026-07-20 裁定,收口 product_210 与本文的归面张力)**:工具发现是**产品↔产品 S2S**(供给方向调用方声明工具清单),按本节铁律"所有 S2S 只走 tailnet、绝不公网",此端点绑 tailscale 接口 + S2S 鉴权,**不出边缘**。边缘面的 `/.well-known/*` **仅**放浏览器相关(如 `openid-configuration`),不含 `vxture-tools`。product_210 §4.2 的工具清单端点据此明确为 tailnet 面。
...
| 回函 03 §3                    | 答复                                                                                                                                                          |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. 内网 auth-bff base URL     | §2 已定:worker-01 tailnet 的 S2S 换票口(2026-08-10 更正过一次,见 §2;端口见[端口登记表](https://claude.ai/code/artifact/0f44735a-c6bc-4881-a440-3446a2411a5f)) |
| 2. webhook 投递源             | §3.1:tailnet 投递(源 = worker-01 tailscale IP `<worker-01-tailnet-ip>`;`ARDA_WEBHOOK_BASE_URL` 解耦 + reseed)                                                 |
...
| 阶段                  | 动作                                                                                                                                                                                                                                                                                                               | 归属            | 授权点                                          |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------- | ----------------------------------------------- |
| **P0 边界收口**       | 平台侧:①内网 base = worker-01 tailnet 的 S2S 换票口(§2,2026-08-10 更正过一次);②seed `ARDA_WEBHOOK_BASE_URL` 解耦 + 生产 env(arda tailnet 地址)+ reseed(§3.1);③arda 切换后轮换 `AUTH_INTERNAL_TOKEN`。产品侧(线 B):`PLATFORM_API_URL` 改内网、边缘只放浏览器路由、flush 守卫/内置定时器、出站 host 断言(回函 03 §4) | 平台 + 线 B     | ②为代码/seed 项,单独授权;③ = owner 运维         |
...
| #   | 决策(回函 04 §6)                            | 结论                                                                                                                                                                                                                                                                                       |
| --- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D1  | 内网寻址约定                                | ✅ **采纳**:同 apex S2S 走 tailnet;平台 base = worker-01 tailnet 的 S2S 换票口(auth-bff 既有 tailscale 绑定,零新基建;端口 2026-08-10 更正过一次,见 §2);webhook 同步内网化(`ARDA_WEBHOOK_BASE_URL` 解耦,§3.1);raw tailscale IP = 仓内既有约定(§2);端点路径不变。直接答复回函 03 §3.1/§3.2。 |
