...
> 定位:落地 **product_310 D5** —— 把 `storage.bytes` 从"过渡池"转正为真正的 **gauge(存量)**计量,补齐 `metric_kind = counter | gauge` 的**写路径与读侧**。gauge vs delta/consume 的决策依据见 [`arda_300`](../20-specs/210-arda/40-arda_300_integration-final.md) §2;值域权威见 [`product_220`](./product_220_catalog-resource-model.md) §4。
