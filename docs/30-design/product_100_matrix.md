...
| `runos`      | Runos    | L1           | runos.ai                                                | 是                                                                 | 是                                          | **是**(技能资产,scope=use)            | 平台能力(四原语,scope=use)      | **是**(业务权威在自库 `vx_runos_db`:能力注册表、entitlement 快照、配额计数、审计) | client+product 已 seed;**商业能力面**(v1.2,runos `ADR-003`;原"多模态助手 agent"、"纯控制面"文案均作废)                                               |
...
**不在本次范围**:域名 `runos.ai` → `runos.vxture.com` 与中文名归改名/注册线 issue [#205](https://github.com/vxture/vxture-platform/issues/205)(机械项,本表不重复处理);商业域内的计量归属与去重、org 自建能力的 grant 求值点为**未决项**,登记在 `product_110` §6.8,**商业域首个可售能力包上线前**须收口。
