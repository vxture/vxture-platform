...
> 📌 **后记（2026-07-06）**：PLG 自动建租决策继续有效；但本文的"单层租户"语境已被四层稳定模型（User → Tenant → **Workspace** → 两级 Membership）扩展——注册即建 personal tenant + 1 个 default workspace（`data_identity_200_schema.md`），订阅/权益/隔离主体为 workspace（ADR-11），产品分层与共享语义见 ADR-12 + `docs/30-design/product_110_sharing-isolation.md` v1.0。本 ADR 按惯例不改写，读者以上述文档为现行权威。
