# 运行 AI 审计

> 操作手册：如何对 Vxture 仓库执行合规审计
> 更新：2026-05-11

---

## 触发时机

| 场景                             | 操作                             |
| -------------------------------- | -------------------------------- |
| PR 合并前（新增包 / 重构层边界） | 手动触发                         |
| 大版本发布前                     | 手动触发                         |
| CI 流水线（可选门控）            | 见 [ci-gate.md](./30-ci-gate.md) |

## 执行方式

向 Claude Code 发送以下指令：

```
请读取 docs/40-implementation/ai/audit/prompt.md，然后对以下包执行完整的合规审计：
[包名列表，或写"所有已完成包（见 scope.md）"]
```

Claude 将：

1. 读取 `scope.md` 确认审计范围
2. 按 `rules/index.md` 顺序依次执行 01–10 规则
3. 生成报告，文件名格式 `ai-rules-audit-YYYY-MM-DD.md`
4. 按 `severity.md` 分级（P0–P3）

## 审计范围

已完成包（必须审计）的定义见 [scope.md](./70-scope.md)。

未完成包（⚠️ 标注的）跳过审计，直到标记为完成。

## 查看结果

报告输出至对话或指定目录。严重级别：

| 级别 | 含义                  | 阻塞合并      |
| ---- | --------------------- | ------------- |
| P0   | 架构违规 / 层边界破坏 | ✅ 是         |
| P1   | 依赖规则违规          | ✅ 是         |
| P2   | 代码规范问题          | ❌ 否（警告） |
| P3   | 注释 / 风格建议       | ❌ 否（提示） |

详见 [severity.md](./80-severity.md)。
