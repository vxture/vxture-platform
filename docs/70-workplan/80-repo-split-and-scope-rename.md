...
| #   | 任务                                                                                                                                                                                               | 状态                                     |
| --- | ---                                                                                                                                                                                                | ---                                      |
| 4-1 | 给 `product_240` §2.2 与 §6#17 补「公开仓 或 付费计划」前置条件                                                                                                                                    | ✅ **已做**                              |
| 4-2 | `IMAGE_PRIMARY_REGISTRY` **已接线**：值 `acr` → **`aliyun-acr`**（`acr` 与 Azure Container Registry 同名，在有两个源的语境里会误读）；主机侧选源从**写死的偏好**改为按该变量定主源、另一源降为兜底 | ✅ **已做**                              |
| 4-3 | `TAILSCALE_AUTHKEY` **确认可 revoke**（见下）；`VXTURE_NPM_REGISTRY` **保留**（5 个产品仓在用）                                                                                                    | ✅ **已做**                              |
| 4-4 | `DEPLOY_WORKER02_*` 的授权仓                                                                                                                                                                       | ✅ **已确认：授权给 5 个仓且在用，保留** |
| 4-5 | 公网域名是代码层常量 —— 已落 `docs/50-deployment/01-environments.md` §一                                                                                                                           | ✅ **已做**                              |
...
跨 `vxture` 组织全部 19 个仓逐一解出 workflow 内容 grep，**四项里只有一项真是死凭据**：

| 项                                    | 原判       | 实测                                                                      | 处置                  |
| ------------------------------------- | ---------- | ------------------------------------------------------------------------- | --------------------- |
| `TAILSCALE_AUTHKEY`                   | 疑似零引用 | **全组织零引用**                                                          | 可删，待 owner revoke |
| `VXTURE_NPM_REGISTRY`                 | 疑似零引用 | **5 个产品仓在用**（arda / karda / umbra / vxtpl / yucer 的 `build.yml`） | 保留                  |
| `NODE_AUTH_TOKEN`                     | 疑似零引用 | 拆仓后成为**必需品**；2026-08-21 曾因过期导致 CI 全线 401                 | 保留，纳入轮换周期    |
| `PROMOTION_TOKEN` / `PROMOTION_ACTOR` | 已废       | 两组织均已不存在                                                          | 无需动作              |

#### 4-3 补充排查：`TAILSCALE_AUTHKEY` 为什么确实可以 revoke

owner 提出「没有应用未必没有价值」，故不止查引用，还查了它**可能的两条用途**：

| 用途                | 现状                                                                                                                        |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| CI 入 tailnet       | 已改用 **OAuth client**（`TAILSCALE_OAUTH_CLIENT_ID/SECRET` + `_TAG`），全组织 19 仓零引用                                  |
| 主机 bootstrap 入网 | **人工步骤**——`deploy-manual-init/bootstrap/11-bootstrap-host.sh` 只打印提示让人手输 `--authkey=<key>`，**不读这个 secret** |

两条用途都不经由 GitHub secret，故可 revoke。

**但排查同时发现真正的遗留物**：`docs/50-deployment/rebuild/10-README.md` 的 org 级密钥清单
**仍把它列为必需**。这比 secret 本身更值得清——不清的话，下一个照该文档建仓的人会配一个
没人用的凭据，而"文档要求配"会让后来者不敢删。已同步移除，并把该文件里的环境级密钥
清单一并回写为 `DEPLOY_HOST_*`（此前还是旧名）。

**值得记的不是清掉了什么，是差点清错什么。** 若按「疑似即清」执行，会打断五个产品仓的 CD，
