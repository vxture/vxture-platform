...
| 4-2 | `vars.IMAGE_PRIMARY_REGISTRY`（值 `acr`，仅新仓 repo 层，仓内零引用）    | ⏳ **待 owner**：删或说明消费方            |
| 4-3 | `TAILSCALE_AUTHKEY` / `VXTURE_NPM_REGISTRY`                              | ✅ **已全域确认**，结论见 4-6              |
...
| 4-8 | `DEPLOY_KNOWN_HOSTS` fail-closed —— 7 处 SSH/SCP 调用已接 `fingerprint`  | ⏳ **接线完成，待 owner 配 secret 才生效** |
