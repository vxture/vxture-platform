> ⚠️ **数据模型部分已过时（2026-07-01）** — 平台数据架构以 **[`../../design/data_platform_100_architecture.md`](../../design/data_platform_100_architecture.md)** 为权威，本文所述 schema 设计勿再参考。（注：`@vxture/core-database` **包本身仍在用**——其运行时 Prisma 客户端从统一 schema 重生成，见 v2 §18.1.4；包 API 文档另行维护，不随本文退役。）
