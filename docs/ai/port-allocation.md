...
本节只维护端口编号，避免本地开发和跨仓业务冲突；不代表 `vxture` 仓库负责部署这些服务。vx-worker-02 上的业务 beta/prod 部署由外部业务仓库维护，边界见 [`docs/deployment/08-code-environment-map.md`](../deployment/08-code-environment-map.md)。
