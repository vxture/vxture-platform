...
**偏离纪律（2026-07-21 增）**：标准条款因前置依赖未就绪（如共享包未发布、平台端点未上线）而暂不可满足时，
**不得静默自行发挥**。必须三步：① 实现处显式标注偏离与原因（代码注释引用被偏离的标准条款）；② 本仓技术债
登记处记名 TD 条目（条款 / 原因 / 回收条件）；③ 向平台线回报，由平台确认"过渡态可接受"或"先补标准"。
前置就绪后按登记回收，平台发布依赖更新时会据登记逐仓催收。**静默偏离 = 未达标**（整顿验收不通过；本条为
PR 评审比对项，不设新机检）。实例教训：vxtpl 在 `@vxture/shared@1.5.0` 未发包时自写 health identity 组装
且未申报，与标准 025"共享助手禁止各写一份"条款的冲突直到平台侧核查 arda 来函时才后置发现（2026-07-21）。
...
- **同批交付硬性要求（2026-08-10 起）**：**凡改动 `deploy/database/ddl/[0-9]*.sql` 的 PR，必须同批携带
  对应的幂等活库增量**（内嵌 seed，见上条），否则不得合并——这是 PR 评审的比对项。改 DDL 而不补增量
  = 新库有、活库没有，**而代码是按新结构写的**，于是功能在生产上直接坏掉，却不会有任何构建或测试报错。
  实例：`#186` 声明 `promotion.voucher_redemptions.redemption_no` 并同批让 admin-bff 查询该列，DDL 那半
  从未落活库，生产核销台账因此报 `column does not exist` 长达两周（2026-08-07→08-10 修复）。
  - **判据（机器可验）**：diff 命中 `ddl/[0-9]*.sql` 且 seed 侧无对应幂等语句 = 未达标。
  - 只加注释、不改结构的 DDL 变更不在此列（会改指纹但无需增量，PR 描述里说明即可）。
- **`[B0]` 指纹红灯纪律**：基线校验的 `[B0] DDL baseline hash mismatch` 表示**活库结构与 DDL 文件不一致**。
  **禁止直接 `restamp-ddl-baseline` 消红**——那个动作只重打指纹、不改结构，用在定位差异之前等于把真实缺口
  永久盖掉（且盖掉后再也没有信号）。正确顺序**恒为三步**：
  1. **定位差异**：`git log` 查活库盖章时间之后改过哪些 `ddl/[0-9]*.sql`，逐项到活库查该结构是否存在；
  2. **补齐**：缺什么补什么（幂等增量内嵌 seed → db-init `seed`），并**读活库逐项确认落地**；
  3. **再 restamp**：此时活库确实等于 DDL，重打指纹是陈述事实。
     同一条 restamp 命令，第 2 步之前执行是掩盖、之后执行是记录——**区别只在于有没有先验证**。
     推论：**任何"最省事的做法恰好是消掉告警"的情形，都要先证明告警是假的**，再考虑消它。
...
- [ ] **仓库骨架 + `docs/` 编号分类**对齐模板。
- [ ] **无静默标准偏离**：暂不可满足的条款均已三步登记（实现处标注 + TD 记名 + 回报平台），且带回收条件。
