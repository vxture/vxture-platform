...
`@vxture/design-ui` 的 `src/components/` 按**来源与业务含量**分目录，不按页面或产品分：

| 目录        | 收什么                                  | 判据                                                        |
| ----------- | --------------------------------------- | ----------------------------------------------------------- |
| `ui/`       | shadcn 上游有对应件的基础组件           | 上游 registry 里有同类件。结构照上游，取值绑 T2，定制就地做 |
| `patterns/` | 上游没有的组合件                        | **产品实据**：已在 console / admin / opera 中被各自重写过   |
| `layout/`   | container / stack / grid 一类纯排布容器 | 无视觉、只管排布                                            |
| `_pending/` | **临时**：待重写或待删除的存量件        | 目录会被清空并删除，不接受新增                              |

三条硬边界：

1. **`ui/` 的定制必须留痕**。改了上游哪一处、为什么，写在文件头。没有出处的偏离，
   下一个人无从判断是有意还是手滑。
2. **`patterns/` 的收录门槛是实据不是设想**。一个图案要进来，先证明产品已经各写了一遍；
   "以后可能会用到"不是理由——那正是造出一堆零消费组件的来路。
3. **DS 零业务**。图案只认形状不认业务含义：`StatusBadge` 有 tone，没有"订阅已逾期"；
   `DataTable` 有列定义，没有"租户列表"。带业务归属的共享面板（UserPanel / TenantPanel
   一类）不进本包，归本仓的 `@vxture/domain-ui`（`private: true`，不发布）。

产品专名一律禁止（D16）：没有 `console-*` / `admin-*` / `platform-*` 前缀。

### 1.4 结构件命名：view 为锚，Header / Heading 分性质

**`view` 的名字来自"有导航的页面"这类布局**——那是它最典型的场景，这类页面优先
用这套件。但**名字是来源，不是前提**：组件不依赖导航存在，业务侧没有导航时，
只要形状合适照用。

不叫 `page`：Next 的 `page.tsx` 是路由文件，`PageHeader` 读起来像"路由的头部"；
外壳的顶栏也叫 header，两个 header 撞在一起谁也说不清指哪个。

`content` 这个词**保留**，留给"view 内部的内容区"一类后续概念，现在不占用。

两个后缀分性质，不可混用：

- **`*Header`** —— **区域**：标题 + 描述 + 动作区，有自己的布局
- **`*Heading`** —— **纯标题**：标题 + 可选描述，不带动作

标题阶梯由 `SectionHeader` 的 `level` 一个参数给出，**语义元素与排版角色同步**，
不会各说各话：

| level | 语义元素 | 排版角色    | 通常用在     |
| ----- | -------- | ----------- | ------------ |
| 1     | `h1`     | `heading-2` | 一级         |
| 2     | `h2`     | `heading-3` | 板块标题     |
| 3     | `h3`     | `heading-4` | 板块内的分组 |
| 4     | `h4`     | `heading-5` | 分组内的小节 |

**四档全开，包括 level 1。** DS 的职责到"每一档长什么样"为止——排版角色、语义
元素、两者的对应关系。放几个、放在哪，是信息结构，不在 DS 的题目里；样式与数据
本就分离，这里不存在需要 DS 表态的问题。

不拆成四个组件，是因为多一个组件就多一处"元素和排版对不上"的机会，一个 `level`
参数把两者锁死。
...
| 组件              | 是什么                                                              |
| ----------------- | ------------------------------------------------------------------- |
| `AppBar`          | 应用顶栏，跨 view 不变（属外壳，不在 DS）                           |
| `ViewHeader`      | view 顶部的完整区域：眉标、图标底板、描述、标题旁附加物、右侧动作区 |
| `SectionHeader`   | 纯标题阶梯，可选图标 / 描述 / 动作                                  |
| `ViewLayout`      | view 的纵向骨架，板块之间 `gap-xl`                                  |
| `Section`         | 板块容器，内部 `gap-md`；头部复用 `SectionHeader`                   |
| `SplitViewLayout` | 左导航 + 右内容的分栏，窄屏塌成单列                                 |
| `SectionNav`      | 板块间导航                                                          |

`ViewHeader` 与 `SectionHeader` 的分工是**结构繁简**，不是层级高低：要那套完整
区域用前者，只要一行标题用后者。

文案怎么写、与导航项是否同名，属于产品的信息设计，不在本规范内。
