...
**`view` 指有导航的页面**——它与导航项一一对应，进入 view 的唯一入口是点导航。
...
| 层    | 组件            | 语义元素 | 用在哪                               | 每 view 数量 |
| ----- | --------------- | -------- | ------------------------------------ | ------------ |
| —     | `AppBar`        | —        | 应用顶栏，跨 view 不变（属外壳）     | 1            |
| **1** | `ViewHeader`    | `h1`     | 内容区一级标题，**文案等于导航项名** | **1**        |
| **2** | `SectionHeader` | `h2`     | 板块标题区，可带板块级动作           | n            |
| **3** | `GroupHeading`  | `h3`     | 板块内分组小标题，纯文本             | n            |

配套容器：`ViewLayout`（view 的纵向骨架）、`Section`（板块容器）、
`SplitViewLayout`（左导航 + 右内容的分栏）、`SectionNav`（板块间导航）。

**一个 view 只能有一个 `ViewHeader`，且文案必须等于导航项名。** 两处不一致时用户
会怀疑自己点错了——这不是措辞偏好，是导航可信度问题。
